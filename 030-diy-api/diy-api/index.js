...
app.get("/filter", (req, res) => {
  const filteredJokes = jokes.filter((joke) => joke.jokeType === req.query.type);
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const jokeExisting = jokes.find((joke) => joke.id === id);
  const replacementJoke = {
    id: id,
    jokeText: req.body.text || jokeExisting.jokeText,
    jokeType: req.body.type || jokeExisting.jokeType,
  };
  const jokeIndex = jokes.findIndex((joke) => joke.id === id);
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (jokeIndex > -1) {
    ...
    res.status(200).json({ deletedJoke: deleteJoke });
  } else {
    res
      .status(404)
